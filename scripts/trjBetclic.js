...
function logMatchInfo(marketElement) {
...
function addTRJToMarket(marketElement) {
...
function processMarketElements() {
  ...
  marketElements.forEach((marketElement, index) => {
    try {
      if (!marketElement.hasAttribute("data-trj-processed")) {
        ...
        logMatchInfo(marketElement);
        addTRJToMarket(marketElement);
...
function addTRJToMatchPage() {
  const marketBlock = document.querySelector(
    "sports-match-markets sports-markets-single-market .marketBox.is-table"
  );

  if (marketBlock) {
    addTRJToMarket(marketBlock);
  } else {
    console.log("Bloc de marché de match non trouvé.");
  }
}

export function observeMarkets() {
  processMarketElements(); // Traiter les éléments déjà présents au chargement de la page

  const observer = new MutationObserver((mutationsList) => {
    for (const mutation of mutationsList) {
      if (mutation.type === "childList" || mutation.type === "subtree") {
        processMarketElements(); // Traiter les nouveaux éléments ajoutés dynamiquement
      }
    }
  });
  ...
  console.log("Observation des changements dans le DOM activée.");
}

export function observeMatchPage() {
